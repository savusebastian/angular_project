...
def pcmadd_DB():
	...
	URL = 'https://www.pcmadd.com/dell-optiplex-7010-intel-pentium-g2130-3-20-ghz-hdd-250-gb-ram-4-gb-unitate-optica-dvd-video-intel-hd-graphics-2000-sff.html'
		...
		try:
			...
			for item in schemaorg_data:
				...
				if item.get('itemprop') == 'price':
					data[item.get('itemprop')] = item.get_text()[:-3]

				if item.get('itemprop') == 'priceCurrency' or item.get('itemprop') == 'sku':
