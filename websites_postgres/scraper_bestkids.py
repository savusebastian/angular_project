...
def bestkids_DB():
	...
	URL = 'https://www.bestkids.ro/p-56172-jucarie-interactiva-lama-dansatoare-boppi.aspx'
	...
	for link in links:
		try:
			...
			for item in schemaorg_data:
				...
				if item.get('itemprop') == 'brand' or item.get('itemprop') == 'sku':
				...
				if item.get('itemprop') == 'image':
					data[item.get('itemprop')] = 'https://www.bestkids.ro/' + item.get('src')
