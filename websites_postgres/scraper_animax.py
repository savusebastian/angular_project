...
def animax_DB():
	...
	URL = 'https://www.animax.ro/haina-doua-fete-red-45cm.html'
	...
	for link in links:
		try:
			...
			if result != []:
				...
				# print('Insert', link)
				cur.execute("INSERT INTO product(%s, %s, %s, %s, %s, %s) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')" % ('url', 'shop', 'product_name', 'sku', 'price', 'price_currency', link, shop, data['name'], data['sku'], data['price'], data['priceCurrency']))
				con.commit()
