...
def stickere_DB():
	...
	URL = 'http://www.stickere.net/sitemap.xml'
	...
	for link in links:
		try:
			...
			for item in schemaorg_data:
				if item.get('itemprop') == 'name' or item.get('itemprop') == 'availability':
					data[item.get('itemprop')] = item.get('content')

				if item.get('itemprop') == 'price':
					data[item.get('itemprop')] = item.get_text().strip()[:-4]
					data['priceCurrency'] = item.get_text().strip()[-3:]
