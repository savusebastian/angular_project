	...
	for link in links[182:]:
		try:
			...
			for item in schemaorg_data:
				...
				if item.get('itemprop') == 'priceCurrency' or item.get('itemprop') == 'price' or item.get('itemprop') == 'sku' or item.get('itemprop') == 'productID':
			...
			if result != []:
				...
				# print('Insert', link)
				cur.execute("INSERT INTO product(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % ('url', 'shop', 'product_name', 'image', 'brand', 'model', 'sku', 'price', 'price_currency', 'availability', link, shop, data['name'], data['image'], data['brand'], data['model'], data['sku'], data['price'], data['priceCurrency'], data['availability']))
				con.commit()
