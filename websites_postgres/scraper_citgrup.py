...
def citgrup_DB():
	...
	URL = 'https://www.citgrup.ro/workstation/second-hand/hp_z820/53573.html'
		...
		try:
			...
			for item in schemaorg_data:
				...
				if item.get('itemprop') == 'price':
					data[item.get('itemprop')] = str(float(item.get_text()[:6].replace('.', '')) + (19 / 100 * float(item.get_text()[:6].replace('.', ''))))
