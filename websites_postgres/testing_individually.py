...
def case_smart_DB():
	...
	URL = 'https://www.case-smart.ro/product-sitemap.xml'
	...
	for link in links[1:]:
		try:
			...
			cur.execute(f"SELECT sku FROM product WHERE sku = '{data['sku']}'")
			result = cur.fetchall()

			if result != []:
				# print('Update', link)
				cur.execute(f"UPDATE product SET price = '{data['price']}' WHERE sku = '{data['sku']}'")
				con.commit()

			else:
				# print('Insert', link)
				cur.execute(f"INSERT INTO product({'url'}, {'shop'}, {'product_name'}, {'image'}, {'sku'}, {'price'}, {'price_currency'}, {'availability'}) VALUES ('{link}', '{shop}', '{data['name']}', '{data['image']}', '{data['sku']}', '{data['price']}', '{data['priceCurrency']}', '{data['availability']}')")
				con.commit()
