	...
	for link in links[54:]:
		try:
			...
			if result != []:
				...
				# print('Insert', link)
				cur.execute("INSERT INTO product(%s, %s, %s, %s, %s, %s) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')" % ('url', 'shop', 'product_name', 'price', 'price_currency', 'availability', link, shop, data['name'], data['price'], data['priceCurrency'], data['availability']))
				con.commit()
