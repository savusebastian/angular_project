...
def avmall_DB():
	...
	collection = db['avmall_products']

	URL = 'https://www.avmall.ro/sitemap.xml'
	...
	for link in links[1:]:
		try:
			...
			jsons = len(web_soup.find_all(type='application/ld+json'))
			...
			for item in split_data:
				...
				if item == 'sku' or item == 'mpn' or item == 'priceCurrency' or item == 'image' or item == 'brand':
					data[item] = split_data[i + 2]

				if item == 'price':
					data[item] = split_data[i + 1].strip()[1:-1]
