...
def citgrup_DB():
	...
	collection = db['citgrup_products']

	URL = 'https://www.citgrup.ro/sitemap.xml'
	...
	for link in links:
		try:
			...
			for item in schemaorg_data:
				...
				if item.get('itemprop') == 'availability' or item.get('itemprop') == 'brand':
					data[item.get('itemprop')] = item.get_text().strip()

				if item.get('itemprop') == 'priceCurrency' or item.get('itemprop') == 'productID':
					data[item.get('itemprop')] = item.get('content')

				if item.get('itemprop') == 'price':
					data[item.get('itemprop')] = str(float(item.get_text().split(' ')[0].replace('.', '')) + round(19 / 100 * float(item.get_text().split(' ')[0].replace('.', ''))))
