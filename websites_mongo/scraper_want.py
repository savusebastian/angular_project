...
def want_DB():
	...
	collection = db['want_products']

	URL = 'https://www.want.ro/servicii-it/318-printare-copiere-a4-color-100-pagini-86900.html'
	...
	for link in links:
		try:
			...
			for item in schemaorg_data:
				...
				if item.get('itemprop') == 'sku' or item.get('itemprop') == 'price' or item.get('itemprop') == 'priceCurrency':
