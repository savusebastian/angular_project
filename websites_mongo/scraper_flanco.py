...
def flanco_DB():
	...
	collection = db['flanco_products']

	URL = 'https://www.flanco.ro/sitemap_product.xml'
	...
	for link in links:
		try:
			...
			for item in schemaorg_data:
				if item.get('itemprop') == 'brand' or item.get('itemprop') == 'priceCurrency' or item.get('itemprop') == 'price':
				...
				if item.get('itemprop') == 'image' and exists_image == False:
					data[item.get('itemprop')] = item.get('data-lazy')
					exists_image = True
