...
def watchshop_DB():
	...
	collection = db['watchshop_products']
	...
	for link in links[76:]:
		try:
			...
			for item in schemaorg_data:
				...
				if item.get('itemprop') == 'priceCurrency' or item.get('itemprop') == 'price' or item.get('itemprop') == 'availability' or item.get('itemprop') == 'brand' or item.get('itemprop') == 'sku' or item.get('itemprop') == 'mpn':
