...
def mycloset_DB():
	...
	collection = db['mycloset_products']

	URL = 'https://www.mycloset.ro/diadora-heritage-mythos-blushield-bright-silver-white'
	...
	for link in links:
		try:
			...
			for item in schemaorg_data:
				...
				if item.get('itemprop') == 'priceCurrency' or item.get('itemprop') == 'price' or item.get('itemprop') == 'image' or item.get('itemprop') == 'sku':
