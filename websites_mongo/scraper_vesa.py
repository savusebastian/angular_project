...
def vesa_DB():
	...
	collection = db['vesa_products']

	URL = 'https://www.vesa.ro/sitemap_products.xml'
		...
		try:
			...
			for item in split_data:
				if item == 'name' and exists_name == False:
					data[item] = split_data[i + 38]
					exists_name = True
					data['slug'] = split_data[i + 38].lower().replace('"', '').replace(',', '').replace('.', '-').replace(' ', '-')
