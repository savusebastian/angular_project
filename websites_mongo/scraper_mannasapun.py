...
def mannasapun_DB():
	...
	collection = db['mannasapun_products']

	URL = 'https://www.mannasapun.ro/sitemap.xml'
	...
	for link in links[7:]:
		try:
			...
			for item in schemaorg_data:
				...
				if item.get('itemprop') == 'brand' or item.get('itemprop') == 'sku' or item.get('itemprop') == 'image':
					data[item.get('itemprop')] = item.get_text().strip()

				if item.get('itemprop') == 'priceCurrency' or item.get('itemprop') == 'price' or item.get('itemprop') == 'availability':
