...
def piticulvesel_DB():
	...
	collection = db['piticulvesel_products']

	URL = 'https://www.piticulvesel.ro/1_ro_0_sitemap.xml'
	...
	for link in links[6:]:
		try:
			...
			for item in schemaorg_data:
				...
				if item.get('itemprop') == 'priceCurrency' or item.get('itemprop') == 'sku' or item.get('itemprop') == 'mpn':
					data[item.get('itemprop')] = item.get('content')

				if item.get('itemprop') == 'price' or item.get('itemprop') == 'brand':
