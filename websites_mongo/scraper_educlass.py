...
def educlass_DB():
	...
	collection = db['educlass_products']

	URL = 'https://www.educlass.ro/sitemap.xml'
	...
	for link in links[127:]:
		try:
			...
			for item in schemaorg_data:
				...
				if item.get('itemprop') == 'price' or item.get('itemprop') == 'availability' or item.get('itemprop') == 'priceCurrency' or item.get('itemprop') == 'mpn' or item.get('itemprop') == 'sku' or item.get('itemprop') == 'brand':
