...
def totalcomenzi_DB():
	...
	collection = db['totalcomenzi_products']

	URL = 'http://www.totalcomenzi.ro/cumpara/organizator-de-pantofi-interconectat-10-compartimente-alb-5833810'
	...
	for link in links:
		try:
			...
			for item in split_data:
				if item == 'Product' and exists_name == False:
					data['name'] = split_data[i + 4]
					exists_name = True
					data['slug'] = split_data[i + 4].lower().replace('"', '').replace(',', '').replace('.', '-').replace(' ', '-')

				if item == 'image' or item == 'model' or item == 'priceCurrency':
